...
from ffedit import curl_builder, logger, process_manager, state_manager, wording
from ffedit.filesystem import get_file_name, get_file_size, is_file, remove_file
from ffedit.hash_helper import validate_hash
from ffedit.types import Commands, DownloadProvider, DownloadSet
...
def validate_source_paths(source_paths : List[str]) -> Tuple[List[str], List[str]]:
	...
	for source_path in source_paths:
		if "nsfw" in source_path or "fairface" in source_path or "yoloface" in source_path or "fan_68_5" in source_path or "2dfan4" in source_path or "xseg_1" in source_path or "bisenet_resnet_34" in source_path or "arcface_w600k_r50" in source_path or "kim_vocal_2" in source_path or "inswapper_128_fp16" in source_path:
...
def resolve_download_url_by_provider(download_provider : DownloadProvider, base_name : str, file_name : str) -> Optional[str]:
	download_provider_value = ffedit.choices.download_provider_set.get(download_provider)
