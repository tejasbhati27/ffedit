...
from ffedit.common_helper import calc_float_step, calc_int_step
from ffedit.face_analyser import get_many_faces
...
from ffedit.types import FaceSelectorMode, FaceSelectorOrder, Gender, Race, VisionFrame
...
from ffedit.vision import normalize_frame_color, read_static_image, read_video_frame
...
def render() -> None:
	...
	FACE_SELECTOR_MODE_DROPDOWN = gradio.Dropdown(
		...
		choices = ffedit.choices.face_selector_modes,
	...
	with gradio.Group():
		with gradio.Row():
			FACE_SELECTOR_ORDER_DROPDOWN = gradio.Dropdown(
				...
				choices = ffedit.choices.face_selector_orders,
			...
			FACE_SELECTOR_GENDER_DROPDOWN = gradio.Dropdown(
				...
				choices = [ 'none' ] + ffedit.choices.face_selector_genders,
			...
			FACE_SELECTOR_RACE_DROPDOWN = gradio.Dropdown(
				...
				choices = ['none'] + ffedit.choices.face_selector_races,
				value = state_manager.get_item('face_selector_race') or 'none'
			)
		with gradio.Row():
			face_selector_age_start = state_manager.get_item('face_selector_age_start') or ffedit.choices.face_selector_age_range[0]
			face_selector_age_end = state_manager.get_item('face_selector_age_end') or ffedit.choices.face_selector_age_range[-1]
			FACE_SELECTOR_AGE_RANGE_SLIDER = RangeSlider(
				label = wording.get('uis.face_selector_age_range_slider'),
				minimum = ffedit.choices.face_selector_age_range[0],
				maximum = ffedit.choices.face_selector_age_range[-1],
				value = (face_selector_age_start, face_selector_age_end),
				step = calc_int_step(ffedit.choices.face_selector_age_range)
			)
	REFERENCE_FACE_DISTANCE_SLIDER = gradio.Slider(
		label = wording.get('uis.reference_face_distance_slider'),
		value = state_manager.get_item('reference_face_distance'),
		step = calc_float_step(ffedit.choices.reference_face_distance_range),
		minimum = ffedit.choices.reference_face_distance_range[0],
		maximum = ffedit.choices.reference_face_distance_range[-1],
		visible = 'reference' in state_manager.get_item('face_selector_mode')
	)
