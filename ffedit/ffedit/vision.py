...
from ffedit.filesystem import get_file_extension, is_image, is_video
from ffedit.thread_helper import thread_semaphore
from ffedit.types import Duration, Fps, Orientation, Resolution, VisionFrame
from ffedit.video_manager import get_video_capture
...
def create_image_resolutions(resolution : Resolution) -> List[str]:
	...
	if resolution:
		...
		for image_template_size in ffedit.choices.image_template_sizes:
...
def create_video_resolutions(resolution : Resolution) -> List[str]:
	...
	if resolution:
		width, height = resolution
		temp_resolutions.append(normalize_resolution(resolution))
		for video_template_size in ffedit.choices.video_template_sizes:
			if width > height:
