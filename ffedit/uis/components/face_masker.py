...
from ffedit import face_masker, state_manager, wording
from ffedit.common_helper import calc_float_step, calc_int_step
from ffedit.types import FaceMaskArea, FaceMaskRegion, FaceMaskType, FaceOccluderModel, FaceParserModel
...
def render() -> None:
	...
	with gradio.Row():
		FACE_OCCLUDER_MODEL_DROPDOWN = gradio.Dropdown(
			...
			choices = ffedit.choices.face_occluder_models,
		...
		FACE_PARSER_MODEL_DROPDOWN = gradio.Dropdown(
			...
			choices = ffedit.choices.face_parser_models,
	...
	FACE_MASK_TYPES_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = ffedit.choices.face_mask_types,
	...
	FACE_MASK_AREAS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = ffedit.choices.face_mask_areas,
	...
	FACE_MASK_REGIONS_CHECKBOX_GROUP = gradio.CheckboxGroup(
		...
		choices = ffedit.choices.face_mask_regions,
	...
	FACE_MASK_BLUR_SLIDER = gradio.Slider(
		...
		step = calc_float_step(ffedit.choices.face_mask_blur_range),
		minimum = ffedit.choices.face_mask_blur_range[0],
		maximum = ffedit.choices.face_mask_blur_range[-1],
	...
	with gradio.Group():
		with gradio.Row():
			FACE_MASK_PADDING_TOP_SLIDER = gradio.Slider(
				...
				step = calc_int_step(ffedit.choices.face_mask_padding_range),
				minimum = ffedit.choices.face_mask_padding_range[0],
				maximum = ffedit.choices.face_mask_padding_range[-1],
...
def update_face_mask_types(face_mask_types : List[FaceMaskType]) -> Tuple[gradio.CheckboxGroup, gradio.CheckboxGroup, gradio.CheckboxGroup, gradio.Slider, gradio.Slider, gradio.Slider, gradio.Slider, gradio.Slider]:
	face_mask_types = face_mask_types or ffedit.choices.face_mask_types
...
def update_face_mask_areas(face_mask_areas : List[FaceMaskArea]) -> gradio.CheckboxGroup:
	face_mask_areas = face_mask_areas or ffedit.choices.face_mask_areas
...
def update_face_mask_regions(face_mask_regions : List[FaceMaskRegion]) -> gradio.CheckboxGroup:
	face_mask_regions = face_mask_regions or ffedit.choices.face_mask_regions
